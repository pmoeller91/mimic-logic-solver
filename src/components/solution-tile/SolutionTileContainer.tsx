...
const SolutionTileContainer = forwardRef<HTMLDivElement, SolutionTileContainerProps>(
  function SolutionTileContainer({ chestSolution, className, chest, contextLabel }, ref) {
    ...
    const totalSolutions = chestSolution.reduce((total, solution) => total + solution[1], 0);
    ...
    const weightedSolutions = chestSolution.map(
      ([contents, amount]) => [contents, amount / contents.length] as [ChestContents[], number],
    );

    // For solutions with multiple possibilities, convert from [[content1,
    // content2], weight] to [content1, weight], [content2, weight].
    const splitSolutions = weightedSolutions.flatMap(([contents, weightedAmount]) =>
      contents.map((content) => [content, weightedAmount] as [ChestContents, number]),
    );
    ...
    const collatedSolutions = Object.entries(
      splitSolutions.reduce(
        (collated, [content, amount]) => {
    ...
    const percentageSolutions = collatedSolutions.map(
      ([contents, amount]) => [contents, amount / totalSolutions] as [ChestContents, number],
    );

    // Sort MIMIC solutions first, then the most likely solutions.
    percentageSolutions.sort((a, b) => {
      if (a[0] === CHEST_CONTENTS.mimic) {
        return -1;
      }
      if (b[0] === CHEST_CONTENTS.mimic) {
        return 1;
      }
      return b[1] - a[1];
    });
