...
const SolutionDisplay = forwardRef<HTMLDivElement, SolutionDisplayProps>(function SolutionDisplay(
  { className, ...rest },
  ref,
) {
  ...
  return (
    <div
      className={clsx("flex flex-grow flex-col items-center justify-center", className)}
      ref={ref}
      {...rest}
    >
      {solution && solutionGrid && <SolutionGrid solution={solution} grid={solutionGrid} />}
      {(!solution || !solutionGrid) && !isSolving && <div>{noSolutionYet}</div>}
      {isSolving && (
