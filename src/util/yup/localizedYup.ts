...
interface YupLocaleConfigConstraint {
  [k: string]:
    | YupLocaleConfigConstraint
    | ((...args: any) => LocalizedValidationError); // eslint-disable-line @typescript-eslint/no-explicit-any
}
...
Yup.setLocale({
  mixed: {
    ...
    required: () =>
      new LocalizedValidationError('Field required', ['yup.mixed.required']),
  },
  number: {
    positive: () =>
      new LocalizedValidationError('Number must be positive', [
        'yup.number.positive',
      ]),
    max: ({ max }) =>
      new LocalizedValidationError(`Number must be less than ${max}`, [
        'yup.number.max',
        { max },
      ]),
    min: ({ min }) =>
