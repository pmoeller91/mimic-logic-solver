...
const resolveRankNoGold: HintResolver<ChestHintTypes['rankNoGold']> = ({
  grid,
  chest,
  gameInfo,
}: HintResolverParams<ChestHintTypes['rankNoGold']>) => {
  const isTruthful = getIsTruthful({ chest, grid, gameInfo });
  const rank = chest.hint.params[0];
  const rankChests = getRankChests({ grid, rank });

  const rankChestsIncludingGold = rankChests.filter((rankChest) =>
    chestContentIncludes({ chest: rankChest, contents: CHEST_CONTENTS.gold })
  );
  const rankChestsAreGold = rankChests.filter((rankChest) =>
    chestContentIs({ chest: rankChest, contents: CHEST_CONTENTS.gold })
  );

  if (isTruthful && rankChestsAreGold.length === 0) {
    rankChests.forEach((rankChest) =>
      chestExcludeContents({ chest: rankChest, contents: CHEST_CONTENTS.gold })
    );
    return true;
  }
